...
import { useGetCategoriesSimple } from "@/api/useGetCategoriesSimple";

const MenuList = () => {
  const { result, loading } = useGetCategoriesSimple();
        ...
        <NavigationMenuItem>
          ...
          <NavigationMenuContent>
            <ul className="grid w-[400px] gap-3 p-4 md:w-[500px] md:grid-cols-2 lg:w-[600px] ">
              {loading && <p className="px-4 py-2">Cargando categorías...</p>}
              {!loading &&
                result.map((category) => (
                  <ListItem
